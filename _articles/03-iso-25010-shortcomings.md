...
Before we dive into details, please allow some nitpicking.
...
While such a hierarchy is structurally simple, it gives rise to serious disadvantages., more on that in a moment.Although such a hierarchy is structurally simple, but brings some disadvantages, more on this shortly.
Alltogether, the standard attempts to establish a widely usable ("generic") taxonomy of almost 40 terms in total. Presumably, all stakeholders should be able to recognize their goals and requirements represented there. In reality though, this is not the case, as you will see right away.
...
The diagram shows the top-level characteristics of ISO 25010 from the  2011 version, which is still the officially valid version. 


![ISO-25010-v2011-top-level-structure](/images/articles/iso-25010/ISO-25010-top-level-2011.png)
Figure 1: ISO 25010, top-level
...
Well – if there are any product owners among you, they will likely miss "Time-to-Market" because you surely want to bring your product to the market or to your users in a timely manner.
If you think that something like Time-to-Market is not a quality attribute at all: It is, however, an attribute that some stakeholders want from a system, and thus it counts as a quality attribute for these people.
...
However in a standard-document as detailed as ISO 25010 presents itself in many parts, there would have certainly been room for those missing pieces.




### Confusion under the Hood
Let's dive into details, and head over to some of the around 40 so-called sub-characteristics of ISO-25010.
...
[^spanish]:For readers like myself who lack Spanish language skills: “I don’t understand that’.”
...
* “Testability”: ISO positions testability under maintainability, which is understandable: When maintaining or changing systems, testability is an important property, to reduce risks. But: From my experience, people consider testability also an indispensable part of reliability. Therefore, testability should occur in both subtrees, not only under maintainability.
* “Availability” is located below reliability. However, if a system is not available, it is not usable. Therefore, it also belongs to usability.
...
These and other problems with ISO 25010 have been initially pointed out by Len Bass et al. [^bass]
...
After all, ISO has had time since 2011 to come up with an improved proposal.
...
From my perspective, these two sentences basically express the same things. 
...
Although internal structure, legibility and conciseness have been part of previous quality models (Boehm mentioned these qualities in his model from 1978!), the ISO refused (or forgot?) to include such properties in their model.
...
Another potential source of unwanted complexity is the ISO metamodel, which distinguishes Quality-in-Use, Data-Quality, Service Quality and Product Quality.
In my opinion, this makes the ISO 25010 model overly difficult to digest for real-world projects.



My final complaint is the paywall: 
The price of almost 140 Swiss Francs (approx. €140 and US-$140) might prevent people from reading the complete document. 
...
ISO 25010 is neither one nor the other.
...
Instead of nearly 40 terms at various levels, a limitation to a few top-level features would be sufficient, coupled with suggestions or examples for specific quality requirements.




In this respect, Len Bass and Co. have done an outstanding job with their fundamental work [^bass]. 
They had already invented the concept of "quality scenarios" many years ago.
...
### Acknowledgements
Thanx to “m” for dramatically improving the quality of this post :-)
