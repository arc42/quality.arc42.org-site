...
export class GraphRenderer {
  ...
  constructor(container) {
    ...
    this._labelVisibilityThreshold = (d) =>
      (isQuality(d) || isRequirement(d) || isStandard(d) ? 1.2 : 0.8) / (d.size / 10);
    ...
    this._hideIfDimmedPropertyUnderStd = (d) =>
      this._isStdSelectionActive() && (isProperty(d) || isDimension(d)) && d._dimmed;
  ...
  _clearEdgeHoverFlags() {
    ...
    if (Array.isArray(this.virtualEdgesData))
      this.virtualEdgesData.forEach((vl) => {
  ...
  _clearNodeLabelHighlightStates() {
    ...
    if (this.labels) {
      this.labels.classed("highlighted", false).classed("connected-highlighted", false);
    }
  }
  ...
  _updateNodesLegendHidden(typeVis) {
    this.nodes.each(function (d) {
      ...
      d._legendHidden =
        (t === NODE_TYPES.QUALITY && !typeVis.quality) ||
  ...
  _hideIsolatedPropertyNodes(typeVis) {
    ...
    this.nodes?.each(function (d) {
      ...
      for (const nbId of nbs) {
        ...
        const isRelevant =
          nb &&
          (nb.qualityType === "quality" || nb.qualityType === "requirement") &&
          !nb._legendHidden;
        if (isRelevant) {
      ...
      if (!hasVisibleNeighbor && considerVirtual) {
        hasVisibleNeighbor = renderer._hasVisibleRequirementNeighborViaQuality(
          d.id,
          nodeById,
          neighbors,
        );
        ...
        nbs.forEach((qId) => {
          ...
          if (standardsVisible) {
            ...
            qNbs.forEach((pId) => {
              ...
              if (
                !pNode ||
                (pNode.qualityType !== NODE_TYPES.PROPERTY &&
                  pNode.qualityType !== NODE_TYPES.DIMENSION)
              )
                return;
  ...
  render(graphData, onNodeHover, onNodeDoubleClick, onNodeClick) {
    ...
    this.virtualLinksLayer = this.svg
      .append("g")
      .attr("class", "virtual-links")
      .style("display", "none");
    ...
    this.labels = this.svg
      ...
      .attr(
        "fill",
        (d) =>
          d.textColor || (isDimension(d) || isProperty(d) ? GRAPH_COLORS.propertyLabelText : null),
      )
    ...
    this.updateLabelVisibility = this.createLabelVisibilityUpdater(
      this.labels,
    ...
    if (this.nodes && this.links && this.labels) {
      if (active) {
        ...
        this.links.each(function (l) {
          ...
          l._canvasHidden = !!(
            selected.isStandard &&
            (renderer._hideIfDimmedPropertyUnderStd(l.source) ||
              renderer._hideIfDimmedPropertyUnderStd(l.target))
          );
        });
        if (Array.isArray(this.virtualEdgesData))
          this.virtualEdgesData.forEach(function (l) {
            const endpointsRelated = renderer._edgeEndpointsRelated(l, selected);
        ...
        if (Array.isArray(this.virtualEdgesData))
          this.virtualEdgesData.forEach((l) => {
            l._dimmed = false;
          });
  ...
  setupDrag() {
    ...
    this.labels.filter((d) => isRoot(d) || isProperty(d) || isDimension(d)).call(dragBehavior);
  }
  ...
  _drawNormalLinks(ctx, linksData) {
    linksData.forEach((l) => {
      ...
      if (
        this._isStdSelectionActive() &&
        (this._hideIfDimmedPropertyUnderStd(l.source) ||
          this._hideIfDimmedPropertyUnderStd(l.target))
      )
        return;
      const isHover = !!l._hoverHighlight;
      const isFilterRelated =
        this.isFiltering && !(l.source?.filterMatched || l.target?.filterMatched);
