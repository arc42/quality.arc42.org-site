...
export class GraphDataProvider {
  ...
  filterByTerm(filterTerm, options = {}) {
    ...
    const requirementNodeIds = this._getRequirementNodeIds(
      nodesPool,
      edgesPool,
      filteredNodeIds,
      connectedNodeIds,
    ...
    const effectiveConnected = this._getEffectiveConnected(
      nodesPool,
      connectedNodeIds,
      filteredNodeIds,
      qualitiesHidden,
    ...
    const forceInclude =
      options.forceInclude instanceof Set
    ...
    const restrictedVisible = baseSet
      ? new Set(
          Array.from(allVisibleNodeIds).filter((id) => baseSet.has(id) || id === QUALITY_ROOT_ID),
        )
      : allVisibleNodeIds;
    ...
    this.#filteredEdges = edgesPool.filter(
      (edge) => restrictedVisible.has(edge.source) && restrictedVisible.has(edge.target),
    );
    ...
    this.#filterMatchedNodeIds = new Set(
      Array.from(filteredNodeIds).filter((id) => restrictedVisible.has(id)),
    );
    this.#filterRelatedNodeIds = new Set(
  ...
  _getEdgesPool(baseSet, propertyIds) {
    ...
    return this.#edges.filter(
      (e) =>
        ...
        (propertyIds.has(e.source) && baseSet.has(e.target)) ||
        (propertyIds.has(e.target) && baseSet.has(e.source)),
    );
  }
  ...
  _getFilteredNodeIds(nodesPool, terms) {
    ...
    nodesPool.forEach((node) => {
      ...
      if (
        lowerTerms.some(
          (t) => label.includes(t) || id.includes(t) || labels.some((lv) => lv.includes(t)),
        )
      ) {
  ...
  _getRequirementNodeIds(
    ...
    const allRequirementNodes = nodesPool.filter(
      (node) => node.qualityType === NODE_TYPES.REQUIREMENT,
    );
  ...
  _getVisiblePropertyNodeIds(edgesPool, restrictedVisible) {
    ...
    this.#propertyNodes.forEach((propNode) => {
      ...
      edgesPool.forEach((edge) => {
        if (
          edge.source === propNode.id &&
          edge.target !== QUALITY_ROOT_ID &&
          restrictedVisible.has(edge.target)
        ) {
          hasVisibleNeighbor = true;
        }
        if (
          edge.target === propNode.id &&
          edge.source !== QUALITY_ROOT_ID &&
          restrictedVisible.has(edge.source)
        ) {
          hasVisibleNeighbor = true;
        }
      });
  ...
  prepareHomeGraphData() {
    ...
    const homeEdges = this.#edges.filter((edge) => {
      return (
        (edge.source === QUALITY_ROOT_ID &&
          this.#propertyNodes.some((node) => node.id === edge.target)) ||
        (edge.target === QUALITY_ROOT_ID &&
          this.#propertyNodes.some((node) => node.id === edge.source))
      );
    });
    return {
  ...
  filterByStandard(standard) {
    ...
    const matchedQualityNodes = this.#nodes.filter(
      (node) =>
        node.qualityType === NODE_TYPES.QUALITY &&
        Array.isArray(node.standards) &&
        node.standards.map((s) => String(s).toLowerCase()).includes(std),
    );
