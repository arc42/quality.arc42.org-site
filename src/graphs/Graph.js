...
export class Graph {
  ...
  applyEnhancedRadialLayout(rootId, levelRadius = 150) {
    ...
    if (propertyNodes.length > 0) {
      ...
      const { qualityNodesByConnections, propertyConnections } =
  ...
  _positionQualityNodes(qualityNodesByConnections, levelRadius) {
    ...
    const maxConns = Array.from(qualityNodesByConnections.keys()).reduce(
      (a, b) => Math.max(a, b),
      0,
    );
    for (let connCount = 2; connCount <= maxConns; connCount++) {
      if (qualityNodesByConnections.has(connCount)) {
        this._positionMultiPropertyQualityNodes(
          qualityNodesByConnections.get(connCount),
          levelRadius,
        );
      }
    }
   ...
   */
  filter(filterTerm) {
    // NOTE: FullGraph may want legend-aware filtering in the future
