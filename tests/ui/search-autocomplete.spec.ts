...
test('searching "quality" surfaces the definition article first in its group', async ({ page }) => {
  ...
  // Deterministic per src/scripts/site/autocomplete.js's scorer:
  // "What is Quality?" carries `aka: Quality`, a whole-word ALIAS_EXACT
  // match (550) — the strongest signal short of an exact title match, and
  // ranked above TITLE_PREFIX (500) precisely so an exact known synonym
  // outranks an article whose title merely *starts with* "quality" (e.g.
  // "Quality Models").
