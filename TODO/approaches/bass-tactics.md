...
| Done | Approach | One-line description | Source |
|:----:|----------|----------------------|:------:|
| ✅ | Split Module | Refactor a non-cohesive module into several cohesive submodules to localize change — site: "Increase Cohesion" (Split a Module sub-tactic) | B+·★ |
| ✅ | Redistribute Responsibilities | Group similar responsibilities scattered across modules into one place — site: "Increase Cohesion" (Redistribute Responsibilities sub-tactic) | B+·★ |

### ✅ Reduce Coupling

| Done | Approach | One-line description | Source |
|:----:|----------|----------------------|:------:|
| ✅ | Encapsulate | Introduce an explicit interface so access flows through it, hiding internals from dependents — site: "Reduce Coupling" (Encapsulate sub-tactic) | B+·★ |
| ✅ | Use an Intermediary | Break a dependency between two modules by placing a broker/mediator between them — site: "Reduce Coupling" (Use an Intermediary sub-tactic) | B+·★ |
| ✅ | Abstract Common Services | Hide similar elements behind one common abstraction so dependents bind once, not per-element — site: "Reduce Coupling" (Abstract Common Services sub-tactic) | B+ |
| ✅ | Restrict Dependencies | Limit which modules a module may use, via visibility and authorization (e.g. layered architecture) — site: "Reduce Coupling" (Restrict Dependencies sub-tactic) | B+·★ |

### Defer Binding

| Done | Approach | One-line description | Source |
|:----:|----------|----------------------|:------:|
| ✅ | Component Replacement | Swap a component at compile/build time (e.g. via build script or makefile) — site: "Defer Binding" (build-time sub-tactic) | B+·★ |
...
| ✅ | Configuration-Time Binding | Bind values at deployment/startup via configuration — site: "Defer Binding" (deploy/startup configuration sub-tactic) | B+·★ |
| ✅ | Resource Files | Bind values at startup/init from external resource files — site: "Defer Binding" (resource-file sub-tactic) | B+·★ |
...
| ✅ | Polymorphism | Bind method implementations at runtime via late binding — site: "Defer Binding" (runtime polymorphism sub-tactic) | B+·★ |
